...
- [x] 六大認知領域已有三款可實際遊玩的小遊戲：市場買菜（數學）、整理菜籃（執行功能）、來去菜市場（注意力），皆已串接後端 API；其餘三個領域仍為「敬請期待」提示
- [x] 三款遊戲的暫停選單、結算頁樣式、結算頁「退出遊戲」導航（統一回到遊戲首頁六大領域畫面）皆已統一
- [x] 三款遊戲統一解析後端錯誤格式（`{success, data, error:{code, message}}`），共用 `core/network/api_response.dart`

### 進行中

- [ ] 首頁資料串接（問候語姓名、動態牆貼文等改為真實資料，目前為假資料）
- [ ] 後端正式部署固定網域（目前用 Cloudflare Tunnel 臨時網址測試，網址會變動；近期已隨網址更新同步修正過一次）
- [ ] `market_route`（來去菜市場）的 `result` 查詢端點尚未串接
